...
export default function MobileHomePage() {
  ...
  useEffect(() => {
    if (currentUser) {
      ...
      setTimeout(() => {
        ...
        const pendingReminder = allReminders.find(
          (r) => r.status === "pending" || r.status === "sent"
        );

        if (pendingReminder) {
          const vehicle = userVehicles.find(
            (v) => v.id === pendingReminder.vehicleId
          );
  ...
  const loadData = () => {
    ...
    setReminders(
      allReminders.filter((r) => r.status === "pending" || r.status === "sent")
    );
  };
          ...
          <div className="grid grid-cols-3 gap-3">
            <Card className="bg-primary-foreground/10 border-primary-foreground/20">
              <CardContent className="p-4 text-center">
                ...
                <p className="text-xs text-primary-foreground/80">Xe</p>
              </CardContent>
            </Card>
            <Card className="bg-primary-foreground/10 border-primary-foreground/20">
              <CardContent className="p-4 text-center">
                ...
                <p className="text-xs text-primary-foreground/80">Nhắc nhở</p>
              </CardContent>
            </Card>
            <Card className="bg-primary-foreground/10 border-primary-foreground/20">
              <CardContent className="p-4 text-center">
                <p className="text-2xl font-bold text-primary-foreground">
                  {vehicles
                    .reduce((sum, v) => sum + (v.mileage || 0), 0)
                    .toLocaleString()}
                </p>
                <p className="text-xs text-primary-foreground/80">Tổng km</p>
              </CardContent>
            </Card>
          </div>
        </div>
